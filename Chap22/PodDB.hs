...
prepDB dbh =
  do
    ...
    unless ("podcasts" `elem` tables) $
      do
        run dbh "CREATE TABLE podcasts (castid INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT, castURL TEXT NOT NULL UNIQUE)" []
        return ()
...
addPodcast dbh podcast =
  ...
  where
    errorHandler e = do fail $ "Error adding podcast; does this URL already exist?\n" ++ show e
...
addEpisode dbh ep =
  void (run dbh "INSERT OR IGNORE INTO episodes (epCastId, epURL, epDone) VALUES (?, ?, ?)" [toSql (castId . epCast $ ep), toSql (epURL ep), toSql (epDone ep)])
...
updatePodcast dbh podcast = void (run dbh "UPDATE podcasts SET castURL = ? WHERE castId = ?" [toSql (castURL podcast), toSql (castId podcast)])
...
getPodcastEpisodes dbh pc = do
  ...
  where
    convEpisodeRow [svId, svURL, svDone] = Episode {epId = fromSql svId, epURL = fromSql svURL, epDone = fromSql svDone, epCast = pc}