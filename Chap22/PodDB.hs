...
connect fp = do dbh <- connectSqlite3 fp
                prepDB dbh
                return dbh
...
addPodcast dbh podcast =
    ...
    where errorHandler e = do fail $ "Error adding podcast; does this URL already exist?\n" ++ show e
...
addEpisode dbh ep = run dbh "INSERT OR IGNORE INTO episodes (epCastId, epURL, epDone) VALUES (?, ?, ?)" [toSql (castId . epCast $ ep), toSql (epURL ep), toSql (epDone ep)]
                    >> return ()
...
updatePodcast dbh podcast = run dbh "UPDATE podcasts SET castURL = ? WHERE castId = ?" [toSql (castURL podcast), toSql (castId podcast)] >> return ()
...
updateEpisode dbh episode = run dbh "UPDATE episodes SET epCastId = ?, epURL = ?, epDone = ? WHERE epId = ?"
...
removePodcast dbh podcast = do run dbh "DELETE FROM episodes WHERE epcastid = ?" [toSql (castId podcast)]
...
getPodcasts dbh = do res <- quickQuery' dbh "SELECT castid, casturl FROM podcasts ORDER BY castid" []
...
getPodcast dbh wantedId = do res <- quickQuery' dbh "SELECT castid, casturl FROM podcasts WHERE castid = ?" [toSql wantedId]
                             case res of
...
convPodcastRow [svId, svURL] = Podcast {castId = fromSql svId, castURL = fromSql svURL}
...
getPodcastEpisodes dbh pc =do r <- quickQuery' dbh "SELECT epId, epURL, epDone FROM episodes WHERE epCastId = ?" [toSql (castId pc)]
                              return (map convEpisodeRow r)
    where convEpisodeRow [svId, svURL, svDone] = Episode {epId = fromSql svId, epURL = fromSql svURL, epDone = fromSql svDone, epCast = pc}