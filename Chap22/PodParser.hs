...
{- | Given a podcast and a PodItem, produce an Episode -}

item2ep :: Podcast -> PodItem -> Episode
item2ep pc item = Episode {epId = 0, epCast = pc, epURL = enclosureurl item, epDone = False}
...
parse content name = Feed {channeltitle = getTitle doc, items = getEnclosures doc}
    where parseResult = xmlParse name $ stripUnicodeBOM content
          ...
          {- | Some Unicode documents begin with a binary sequence, strip it
...
getEnclosures doc = concatMap procPodItem $ getPodItems doc
    where procPodItem :: Content Posn -> [PodItem]
          procPodItem item = concatMap (procEnclosure title) enclosure
            where title = contentToStringDefault "Untitled Episode" (keep /> tag "title" /> txt $ item)
...
{- | Convert [Content] to a printable String, with a default if the passed
in [Content] is [], signifying a lack of a match. -}
...
whatever Content we have is wrapped in an Element, then use txt to pull the
insides back out. -}
