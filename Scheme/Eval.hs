...
import Data.Array
import Data.Char
import Data.Complex (Complex(..), realPart, imagPart, magnitude, mkPolar)
...
eval env (List [Atom "quasiquote", form]) = evalUnquotes form
    where evalUnquotes form' = case form' of
                                 List [Atom "unquote", form''] -> eval env form''
                                 List items ->
                                   do results <- traverse evalUnquotes items
                                      return $ List results
                                 _ -> return form
       ...
       BadSpecialForm "No true clause in case expression: " form
         ...
         List ((List datums):exprs) ->
           do result <- eval env key
              eq <- return $ mapM (\x -> eqv [result, x]) datums
...
primitives = [("+", numAdd), ("-", numSub), ("*", numMul), ("/", numDivide), ("mod", numMod),
              ("quotient", numericBinop quot), ("remainder", numericBinop rem), ("abs", numAbs),
              ("ceiling", numCeil), ("floor", numFloor), ("round", numRound), ("truncate", numTruncate),

              ("exact?", unaryOp exactp), ("inexact?", unaryOp inexactp),
              ("exact->inexact", toInexact), ("inexact->exact", toExact),

              ("real-part", cRealPart), ("imag-part", cImagPart), ("magnitude", cMagnitude),
              ("make-polar", cMakePolar),

              ("sqrt", sciSqrt), ("exp", sciExp), ("expt", sciExpt), ("ln", sciLn), ("log", sciLog),

              ("acos", sciAcos), ("asin", sciAsin), ("atan", sciAtan), ("cos", sciCos),
              ("sin", sciSin), ("tan", sciTan), ("asinh", sciAsinh), ("acosh", sciAcosh),
              ("atanh", sciAtanh), ("cosh", sciCosh), ("sinh", sciSinh), ("tanh", sciTanh),

              ("number->string", numToString),

              ("symbol?" , unaryOp symbolp), ("char?", unaryOp charp), ("string?" , unaryOp stringp),
              ("number?" , unaryOp numberp), ("float?", unaryOp floatp), ("complex?", unaryOp complexp),
              ("ratio?", unaryOp ratiop), ("bool?", unaryOp boolp), ("list?" , unaryOp listp),
              ("vector?", unaryOp vectorp), ("procedure?", unaryOp procedurep),

              ("char-alphabetic?", charIsAlpha), ("char-numeric?", charIsNumeric),
              ("char-whitespace?", charIsWhitespace), ("char-upper-case?", charIsUpper),
              ("char-lower-case?", charIsLower), ("char-is-both?", charIsBoth),
              ("char-upcase", charToUpper), ("char-downcase", charToLower),
              ("char=?", charBoolBinop (==)), ("char<?", charBoolBinop (<)),
              ("char>?", charBoolBinop (>)), ("char<=?", charBoolBinop (<=)),
              ("char>=?", charBoolBinop (>=)), ("char-ci=?", charBoolBinop' (==)),
              ("char-ci<?", charBoolBinop' (<)), ("char-ci>?", charBoolBinop' (>)),
              ("char-ci<=?", charBoolBinop' (<=)), ("char-ci>=?", charBoolBinop' (>=)),

              ("symbol->string", unaryOp symbol2string), ("string->symbol", unaryOp string2symbol),

              ("=", numBoolBinop (==)), ("<", numBoolBinop (<)), (">", numBoolBinop (>)),
              ("/=", numBoolBinop (/=)), (">=", numBoolBinop (>=)), ("<=", numBoolBinop (<=)),

              ("&&", boolBoolBinop (&&)), ("||", boolBoolBinop (||)),

              ("string=?", strBoolBinop (==)), ("string<?", strBoolBinop (<)),
              ("string>?", strBoolBinop (>)), ("string<=?", strBoolBinop (<=)),
              ("string>=?", strBoolBinop (>=)), ("string-ci=?", strBoolBinop' (==)),
              ("string-ci<?", strBoolBinop' (<)), ("string-ci>?", strBoolBinop' (>)),
              ("string-ci<=?", strBoolBinop' (<=)), ("string-ci>=?", strBoolBinop' (>=)),
              ("string-len", stringLen), ("string-ref", stringRef), ("substring", subString),
              ("string-append", stringAppend), ("string", string), ("string-fill!", stringFill),

              ("list->string", unaryOp list2string), ("string->list", unaryOp string2list),
              ("car", car), ("cdr", cdr), ("cons", cons), ("first", car), ("last", lastBlock),
              ("list-ref", listRef), ("list-tail", listTail), ("list-head", listHead),
              ("eq?", eqv), ("eqv?", eqv), ("equal?", equal),

              ("vector-length", vectorLength), ("vector-ref", vectorRef), ("list->vector", list2vector),
              ("make-vector", makeVector), ("vector->list", vector2list)]

ioPrimitives :: [(String, IOPrimitive)]
ioPrimitives = [("apply", applyProc), ("open-input-file", makePort ReadMode),
                ("open-output-file", makePort WriteMode), ("close-input-port", closePort),
                ("close-output-port", closePort), ("read", readProc), ("write", writeProc), 
                ("read-contents", readContents), ("read-all", readAll), ("print", printobj),
                ("display", printobj), ("write-char", printobj), ("println", println), ("newline", newline),
                ("output-port?", isOutputPort), ("input-port?", isInputPort), ("port?", isPort)]
...
numAdd l = foldl1M (\x y -> numCast [x, y] >>= go) l
    where go (List [Number x, Number y]) = return $ Number $ x + y 
...
numDivide l = foldl1M (\x y -> numCast [x, y] >>= go) l
    where go (List [Number x, Number y])
...
numMod l = foldl1M (\x y -> numCast [x, y] >>= go) l
    where go (List [Number a, Number b]) = return $ Number $ mod' a b
          go (List [Float a, Float b]) = return $ Float $ mod' a b
          go (List [Ratio a, Ratio b]) = return $ Ratio $ mod' a b
          go (List [Complex _, Complex _]) = throwError $
...
cMakePolar [mag, p] = numCast [mag, p] >>= go
    where go (List [Number mag', Number p']) 
...
sciExpt [x, y] = numCast [x, y] >>= go
    where go (List [Number x', Number y']) = return $ Number $
...
symbolp, numberp, floatp, ratiop, complexp, charp, stringp, boolp, listp, exactp, inexactp, vectorp, procedurep
  :: SchemeVal -> SchemeVal
...
charIsAlpha, charIsNumeric, charIsWhitespace, charIsLower, charIsUpper, charIsBoth ::
...
stringAppend ((String x):xs) =
  do rest <- stringAppend xs -- MonadFail is needed in order for it to be written as a one-liner
     let (String rs) = rest
     return $ String $ x ++ rs
...
string ((Character x):xs) =
  do rest <- string xs -- MonadFail is needed in order for it to be written as a one-liner
     let (String rs) = rest
     return $ String $ x:rs
...
makeVector [Number k] = return $ Vector $
                        listArray (0, fromInteger k - 1) $ replicate (fromInteger k) (List [])
makeVector [Number k, fill] = return $ Vector $
...
vectorRef [Vector x, Number n] =
  let k = fromInteger n in
    if fst (bounds x) <= k && k <= snd (bounds x)
    then return $ x ! k
    else throwError $ Default $ "index " ++ show k ++ " is out of bounds"
...
cond env ((List (condition:value:[])):alts) =
  do result <- (eval env condition)
     boolResult <- return $ unpackBool result
...
numCast [Ratio a, b@(Complex _)] = return $
...
numCast [a@(Complex _), Ratio b] = return $
