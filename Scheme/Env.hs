...
import Text.ParserCombinators.Parsec (ParseError) -- | Rifare con Megaparsec


data SchemeVal = Atom String -- Types in Scheme
               | List [SchemeVal]
...
instance Show SchemeVal where
  ...
  show (Complex c) = (showFullPrecision. realPart) c ++
...
getVars envRef = do env <- liftIO $ readIORef envRef
...
makeFunc isMacro' varargs env' params' body' = return $ Func isMacro' (map show params')
                                               varargs body' env'
