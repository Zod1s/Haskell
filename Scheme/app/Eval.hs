...
apply badForm _ =
  throwError $ NotFunction "Expected function, found" (show badForm)
  ...
  where
    go (List [Number mag', Number p'])
      ...
      | otherwise =
          return $
            Complex $
              mkPolar (fromInteger mag') (fromInteger p')
    ...
    go (List [Ratio mag', Ratio p'])
      ...
      | otherwise =
          return $
            Complex $
              mkPolar (fromRational mag') (fromRational p')
...
subString [String s, Number start, Number end]
  | start' >= 0 && end' >= start' && l >= end' =
      return (String (take diff (drop start' s)))
  ...
  where
    ...
    l = length s
    diff = end' - start'
subString [String _, notNum, Number _] =
  throwError $
    TypeMismatch "Number" notNum
subString [String _, Number _, notNum] =
  throwError $
    TypeMismatch "Number" notNum
...
stringAppend ((String x) : xs) =
  do
    rest <- stringAppend xs
    -- MonadFail is needed in order for it to be written as a one-liner
    let (String rs) = rest
    return $ String $ x ++ rs
...
applyProc [func, DottedList args rest] =
  applyProc [func, List (args ++ [rest])]
...
printobj [Character c, Port port] =
  liftIO $
    hPutStr port (c : "")
      >> return (Bool True)
printobj [obj, Port port] =
  liftIO $
    hPutStr port (show obj)
      >> return (Bool True)
