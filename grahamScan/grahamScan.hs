...
start :: [Point] -> Point
start ps = head (sortBy (compare `on` y) $ sortBy (compare `on` x) ps)

sortRelativeTo :: Point -> [Point] -> [Point]
sortRelativeTo p ps = p : [maximumBy maxCond x | x <- groupBy groupCond $ sortBy compCond (delete p (nubBy (==) ps))]
    where compCond = compare `on` (angleTo p)
