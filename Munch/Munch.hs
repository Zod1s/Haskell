...
parseIO :: (Token t, Pretty t, Show a)
...
decimalDigit = satisfies  (\c -> elem c "0123456789")
               "decimal digit (0-9)"

hexDigit :: Parser Char Char
hexDigit = satisfies (\c -> elem c "0123456789abcdefABCDEF")
           "hexadecimal digit (0-9, a-f, A-F)"

lowerLatin :: Parser Char Char
lowerLatin = satisfies (\c -> elem c "abcdefghijklmnopqrstuvwxyz")
             "lower case latin letter (a-z)"

upperLatin :: Parser Char Char
upperLatin = satisfies (\c -> elem c "ABCDEFGHIJKLMNOPQRSTUVWXYZ")
...
indent ind (Parser q) = Parser $ \ref stream ->
  ...
  in case c of
    ...
    Consumed u v ->
      case r of
        Left _ -> (c, r)
        Right _ -> if relation ind ref (u, v)
                   then (c, r)
                   else (Declined, Left $ Simply $
                                   UnexpectedIndentation
                                   (message ind ref (u, v)) (u, v))
...
data Error a e = OneOf [Error a e]
               | Because a (Error a e)
               | Simply e
               deriving (Eq, Show)
...
data BasicError t = UnexpectedEOF (Either String (Maybe t))
...
data Annotation t = Note String (Maybe (Pos t))
...
t@(Perms u bs) <&&> p = Perms Nothing $ (Branch t p) : map insert bs
  where insert (Branch w q) = Branch ((fmap flip w) <&&> p) q
...
permuteIndent :: forall t a. (Token t)
              => Indentation t -> Perms t a -> Parser t a
permuteIndent ind = pind
  where pind :: forall b. (Token t) => Perms t b -> Parser t b
...
permuteIndentSepBy :: forall t a. (Token t)
                   => Indentation t
                   -> Parser t ()
                   -> Perms t a
                   -> Parser t a
permuteIndentSepBy ind = pindsep (pure ())
  where pindsep :: forall b. (Token t)
...
renderTree = render . addPrefix
  where addPrefix :: Tree String -> Tree String
        addPrefix (T x bs) = T x $
                             mapLast (mapRoot ("|- " ++) ("|  " ++))
                                     (mapRoot ("\'- " ++) ("   " ++))
                                     (map addPrefix bs)
...
instance (Pretty a, Pretty e) => Pretty (Error a e) where
  pretty = renderTree . toTree
    where toTree :: (Pretty a, Pretty e) => Error a e -> Tree String
...
wrtRef pt dim rel = Indentation
                    { relation = \ref range ->
                        getRel rel (getDim dim ref) (getDim dim $
                                                      getPt pt range)
                    , message = \ref range ->
                        let labelRef = if compareRef rel
                                       then
                                         ["that of the reference position at"
                                         , pretty ref ]
                                       else []
                        in unwords $
                           [ labelPt pt, labelDim dim
...
wrtPos pos pt dim rel = Indentation
                        { relation = \_ range ->
                            getRel rel (getDim dim pos) (getDim dim $
                                                          getPt pt range)
                        , message = \_ range ->
                            let labelRef = if compareRef rel
                                           then ["that of position"
                                           , pretty pos ]
                                           else []
                            in unwords $
                               [ labelPt pt, labelDim dim
